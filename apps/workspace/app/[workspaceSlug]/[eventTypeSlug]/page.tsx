...
interface PageProps {
  params: Promise<{ workspaceSlug: string; eventTypeSlug: string }>;
}
...
async function getEventTypeBySlug(workspaceId: string, eventTypeSlug: string) {
  try {
    ...
    if (error) {
      console.error('Error fetching event type by slug:', {
        code: error.code,
        message: error.message,
        slug: decodedSlug
      });
    ...
    console.error('Exception in getEventTypeBySlug:', err);
    return null;
  }
}
...
export async function generateMetadata({
  ...
  const eventType = await getEventTypeBySlug(workspace.id, eventTypeSlug);
...
export default async function EmbedBookingPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4">
      <EmbedBookingForm workspace={workspace} eventTypeSlug={eventType.slug} />
    </div>
  );
}
