      ...
      <table className="w-full">
        <thead className="bg-slate-50 border-b border-slate-200">
          <tr className="border border-slate-200">
            ...
            <th className="px-6 py-4 text-left text-sm font-bold text-slate-700 tracking-wider">Role</th>
        ...
        <tbody className="divide-y divide-slate-200">
          {Array.from({ length: SKELETON_ROWS }).map((_, i) => (
            <tr key={i} className="bg-white border border-slate-200">
              ...
              <td className="px-6 py-4"><SkeletonBar className="h-6 w-24 rounded-full" /></td>
              <td className="px-6 py-4"><SkeletonBar className="h-4 w-28" /></td>
